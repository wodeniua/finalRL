...
class BDQNAgent:
    ...
    def get_action(self, state):
        s = (torch.tensor(state[0], dtype=torch.float).to(device),
            torch.tensor(state[1], dtype=torch.float).to(device))

        action = [0, 0, 0]
        if np.random.rand() > AGENT.EPSILON:
            action[0] = np.random.randint(0, ENV.ACTION_SPACE[0])  # bin 索引
            action[1] = np.random.randint(0, ENV.ACTION_SPACE[1])  # priority
            action[2] = np.random.randint(0, ENV.ACTION_SPACE[2])  # rotate
        else:
            list_state_action = self.policy_net(s[0], s[1])  # self.policy_net返回的是什么？？？
    ...
    def optimize_model(self):
        ...
        non_final_next_states_1 = torch.cat([s[1] for s in batch.next_state if s is not None], dim=0).reshape(-1, ENV.ROW_COUNT, ENV.COL_COUNT, 3)

        state_batch_0 = torch.cat([s[0] for s in batch.state], dim=0).reshape(AGENT.BATCH_SIZE, ENV.BIN_MAX_COUNT, 3)
        state_batch_1 = torch.cat([s[1] for s in batch.state], dim=0).reshape(AGENT.BATCH_SIZE, ENV.ROW_COUNT, ENV.COL_COUNT, 3)
...
class DQNAgent:
    ...
    def __init__(self):
        self.replay_memory = ReplayMemory(capacity=AGENT.REPLAY_MEMORY_SIZE)
        self.policy_net = DQN(ENV.BIN_MAX_COUNT * 3, 3, [ENV.BIN_MAX_COUNT, 2, 2]).to(device)
        self.target_net = DQN(ENV.BIN_MAX_COUNT * 3, 3, [ENV.BIN_MAX_COUNT, 2, 2]).to(device)
        self.target_net.load_state_dict(self.policy_net.state_dict())
        self.target_net.eval()
        self.optimizer = optim.RMSprop(self.policy_net.parameters())
